...
1. **Cover geometry has no single authority.** Five implementations, three
   per-page thicknesses (`0.002252`, `0.0025`, `0.002347`), zero verified
   paperback readings. `cover-preflight.ts` emits the string *"Two answers to one
   spine — do not spend until this is resolved."*
2. **Two live spine-repair implementations**, 477 loc / 8 importers and 248 loc /
   5 importers, textually different.
3. **No visual QA gate.** Deterministic Layer 1 exists and names a Layer 2 that
   was never built. Every defect found so far passed Layer 1.
4. **`scripts/` is the production surface and has no design.** 310 tracked, 288
   with no caller, 75 mutating state, 2 destructive.
5. **Generic QA tools wear book-specific filenames**, so the next book copies
   rather than calls.

### P1

6. **`projects.routes.ts` is a 3,092-line god file** with 43 routes.
7. **Cover geometry lives inside a Track A HTML renderer.**
8. **Two cover blueprints** — `src/pipeline/cover/cover-blueprint.ts` (246 loc)
   and `scripts/lib/cover-blueprint.ts` (467 loc). Two `src/__tests__` files
   import the *scripts* copy.
...
11. **Frontend is 3,699 lines of untyped, untested JS in one file.**

### P2

12. Untracked build artifacts at the repository root.
13. Mixed CRLF/LF line endings.
14. 79 of 106 test files sit flat in one directory.
15. **Four tests fail on a clean checkout**, all reading real manuscripts from
