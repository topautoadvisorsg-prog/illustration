...
2. **Environment locked down:**
   ...
   - Zod env validator (`backend/src/env.ts`) — fails fast on missing/malformed vars
   - Placeholder detection — services skipped gracefully when keys absent
3. **Smoke test framework** (`yarn smoke`):
   ...
   - All 6 currently SKIPPED (expected — placeholders in `.env`)
4. **Logger** — Pino with PII redaction, child-logger pattern for `stage` / `book_id` / `page_id` / `correlation_id` bindings.
5. **README hierarchy** — 22 READMEs written, every answering the 5 standard questions:
...
6. **Documentation:**
   - `docs/architecture.md` — system diagram + data flow + component inventory
   - `docs/decision-log.md` — ADRs 000–007
   - `docs/pipeline-spec.md` — distilled invariants + V1 out-of-scope
   - `docs/phase-0-plan.md` — day-by-day spike plan
   - `docs/runbook.md` — failure mode lookup
7. **TypeScript** — strict mode, no `any`, `noUncheckedIndexedAccess`, `verbatimModuleSyntax`. Typecheck passes.
...
───────────────────────────────────────────────────
  ○  Anthropic Claude   SKIPPED  env value is .env.example placeholder
  ○  OpenAI             SKIPPED  env value is .env.example placeholder
  ○  Replicate          SKIPPED  env value is .env.example placeholder
  ○  Supabase           SKIPPED  env value is .env.example placeholder
  ○  Upstash Redis      SKIPPED  env value is .env.example placeholder
  ○  Sentry             SKIPPED  env value is .env.example placeholder
───────────────────────────────────────────────────
...
**Spike 2 — Vertical slice (Chanterelle) Part 1:**
...
2. Build `/spikes/vertical-slice/` script:
   ...
   - Step B: Call Claude with master style block draft → emit image prompt
   - Step C: Call OpenAI gpt-image-1 → save PNG
   - Step D: Call Replicate Real-ESRGAN → save upscaled PNG
   - Step E: Sharp DPI gate → confirm ≥300 DPI at 8.5×11
3. Layout (chapter PDF) deferred to Day 3 — Spike 1 engine bake-off informs how Step F is built
...
1. **Master Style Block draft must be ready by D7.** Will draft during D5–D6 while waiting on PDF engine results, deliver for stakeholder review.
2. **OpenAI org verification for `gpt-image-1`** can take 24–48h. Recommend stakeholder kick this off now even if other keys are pending.
...
4. **Sample manuscript schema** — if real chapters aren't delivered by D2, I'll proceed with a synthetic Chanterelle entry and reverse-engineer the parser when chapters arrive. Low risk; just calibrating expectations.
...
/app/docs/decision-log.md                               (ADRs 000–007)
