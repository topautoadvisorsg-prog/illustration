...
> What to do when X breaks. Living document â€” append entries as new failure modes are discovered.

---

## âš  PRODUCTION REQUIREMENT: durable storage (read first)
...
be used in production â€” Railway's container disk is wiped on every redeploy, so
...
## OpenAI returns 403 for `gpt-image-2`

**Cause:** Organization not verified for image generation.
**Fix:** Complete org verification at platform.openai.com â†’ Settings â†’ Organization.
...
## PDF won't open in Acrobat â€” "format error"
...
**Common causes:**
- Wrong page size (must be 8.625Ã—11.25 for 8.5Ã—11 bleed)
