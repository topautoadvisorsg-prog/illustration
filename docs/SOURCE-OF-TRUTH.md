...
| **Barcode-safe area** | `kdp-spec.ts` — the paperback reserve labelled `platform-decision`, the hardcover rectangle `published-constraint` | 18 cover scripts still reimplement it | Cover validation, `scripts/qa/cover-spec.ts` | **Medium** — stated once now, and the operator CLI uses it as a 2.0×1.2in rectangle rather than a full-width row test; the per-book scripts have not been migrated | Route the scripts through `kdp-spec.ts` | **Partly — Phase 1B**; scripts outstanding |
