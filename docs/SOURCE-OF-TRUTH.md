...
| **Which database a script talks to** | `openOperationalDatabase()` in `backend/src/db/operational-access.ts`. Environment is identified by WHERE credentials are declared — production in `.env`, development in `.env.development.local` — never by inspecting the URL | None. Direct `process.env.DATABASE_URL` assignment is **prohibited** and refused by a tripwire in `db/client.ts` | Every operational script | **Low** | Unchanged | No |
...
| **Database connection** | `openOperationalDatabase()` refuses a production write with no `ProductionWriteGrant`, and `db/client.ts` refuses an off-box connection that nothing declared. |
