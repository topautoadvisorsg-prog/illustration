...
>
> 1. **Redo `CH02_P025` (Raven).** Same `readingFieldText`, no content change — it already reads correctly, the AI just baked a glyph typo ("thc" for "the") into the image on the last attempt. Single render attempt via `createAndRunRender`, download and visually inspect the result (crop/zoom the "What it is" paragraph specifically), confirm the typo is gone.
> 2. **Draft Chapter 3 (Plants)'s revision plan**, using the exact same method as the Chapter 2 Phase 1 write-up above: pull the live chapter text with `backend/scripts/_scratch/extract-manuscript.ts`, read it in full, map it against the Ch3 bullets already listed below (break the 21-entry run with bridges, fix the Forager's Code follow-through, bridge into Medicinal/Deadly, pacing contrast), check `computePaginationCapacity` fit-headroom on every page you plan to touch BEFORE writing prose, then write the literal exact before/after text for every page you intend to touch.
