...
describe('series page heading is data-driven', () => {
  it('reads the series name from config (no hardcoded "THE WILD LANDS SERIES")', () => {
    ...
    // Series brand page heading is "ABOUT THE <series> SERIES", derived from the
    // project's series name (leading article stripped) — never a hardcoded brand.
    expect(policy.title.name).toBe('ABOUT THE OCEAN COLLECTION SERIES');
    expect(policy.title.name).toContain('OCEAN COLLECTION');
    expect(policy.title.name).not.toContain('WILD LANDS');
  });
});
