...
function hardConstraints(spec: WholePageSpec): string {
  ...
  lines.push(
    ...
    '- NO LINE OR BORDER AROUND THE TEXT (critical): do NOT draw any line, rule, outline, box, panel edge, frame, or border AROUND the body text or the reading area. The text sits DIRECTLY on the parchment and must feel natural, like a printed book page — never boxed, outlined, or contained by a drawn line. The blueprint rectangles ONLY show WHERE the text belongs; never render their edges or corners. The illustration simply opens into the calm parchment where the text sits, with no hard edge or seam between them.',
    '- ORANGE DASHED SAFE BOUNDARY (critical) — WHY THIS MATTERS: this is a PRINTED book that is TRIMMED at the edges, so anything past the trim is physically CUT OFF and lost on the finished page. The blueprint has TWO concentric ORANGE DASHED rectangles. Keep EVERY heading, every line of body text, the page number, and every important subject (faces, key features) fully INSIDE the INNER orange line — text that crosses it would be cut off the page. The band BETWEEN the two orange lines is an ILLUSTRATION-ONLY BUFFER (no text). The illustration is MEANT to be cut, so it fills out to and PAST the OUTER orange line and bleeds off the page. Text never touches either line.',
    ...
    '- TEXT SAFETY IS HIGHEST PRIORITY: keeping EVERY piece of text — body, headings, scientific names, captions, educational content — fully INSIDE the inner orange line outranks everything else. If the copy is dense or nearing the line, SIZE THE TEXT DOWN (to the smallest still-clearly-readable book size) so every line fits inside the line. Do NOT shrink, move, or crop the illustration to make room — the TEXT adapts, the illustration stays full.',
    ...
    '- ILLUSTRATION IS FULL-BLEED, NEVER REDUCED: the illustration fills its zones and bleeds off the trim edges as drawn in the blueprint; do NOT shrink it, frame it, or pull it inward to make room for text. When text is dense, the TEXT sizes down to fit inside the inner orange line — the illustration is never made smaller.',
