...
function hardConstraints(spec: WholePageSpec): string {
  ...
  lines.push(
    ...
    '- ORANGE DASHED SAFE BOUNDARY (critical): the attached blueprint has TWO concentric ORANGE DASHED rectangles. EVERY heading, every line of body text, the page number, and every important subject (faces, key features) must stay fully INSIDE the INNER orange line. The band BETWEEN the two orange lines is an ILLUSTRATION-ONLY BUFFER — no text and no important subject may enter it. The illustration fills out to and PAST the OUTER orange line and bleeds off the page. Nothing readable touches either line; keep the text comfortably inside the inner line.',
