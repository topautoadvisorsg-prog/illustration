...
const ARTWORK = `${COVER}/BYNI-cover-wrap-art-A_UPSCALED.png`;
...
const result = await buildCover({
  ...
  artworkName: 'BYNI-cover-wrap-art-A_UPSCALED.png',
...
const STEM = `${COVER}/BYNI-cover-PAPERBACK-${PP}pp${SUFFIX}`;

writeFileSync(`${STEM}.pdf`, result.productionPdf);
writeFileSync(`${STEM}-PROOF.png`, result.proofPng);
writeFileSync(`${COVER}/BYNI-cover-PAPERBACK-manifest${SUFFIX}.json`, JSON.stringify(result.manifest, null, 2));
console.log(`\nproduction -> ${STEM}.pdf`);
