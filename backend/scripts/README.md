...
CLI utilities run via `tsx`. Not part of the runtime — these are operator tools.
...
| OpenAI FAIL `gpt-image-1 NOT in account` | Org not verified for image gen | Complete OpenAI org verification (Settings → Organization) |
...
| Sentry FAIL | Invalid DSN format | Copy DSN from Sentry project → Settings → Client Keys |
