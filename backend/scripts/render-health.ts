...
type Bucket = 'done' | 'dropped' | 'notstarted';
function bucketOf(id: string): { b: Bucket; status?: string; err?: string | null } {
  ...
  if (r.status === 'RENDERING') {
    ...
    return ageMin > STUCK_MIN ? { b: 'dropped', status: `RENDERING stuck ${Math.round(ageMin)}m`, err: r.errorMessage } : { b: 'done', status: 'RENDERING (active)' };
  }
  return { b: 'notstarted', status: r.status };
}

const sectionOf = (k: string) => (k.startsWith('CH') ? k.slice(0, 4) : k.startsWith('FM_') ? 'FRONT' : 'BACK');
const counts = new Map<string, { done: number; dropped: number; notstarted: number }>();
...
for (const p of pages) {
  ...
  const c = counts.get(sec) ?? { done: 0, dropped: 0, notstarted: 0 };
...
for (const [sec, c] of [...counts.entries()].sort()) {
  console.log(`  ${sec.padEnd(7)} done ${String(c.done).padStart(3)}  dropped ${c.dropped}  not-started ${c.notstarted}`);
}
